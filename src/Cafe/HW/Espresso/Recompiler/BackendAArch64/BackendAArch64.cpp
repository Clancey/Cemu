...
class AArch64Allocator : public Allocator
{
  private:
	...
	// We return the RW (writable) pointer to xbyak so it can write code.
  ...
  public:
	...
	uint32* alloc(size_t size) override
	{
		...
		if (!rxPtr)
			return nullptr;
		// Return the writable alias so xbyak can write instructions
	...
	void free(uint32* p) override
	{
		if (!m_freeDisabled && p)
		{
			// Convert RW back to RX for freeing
	...
	{
		// Dual-mapped memory doesn't need mprotect toggling
		return false;
	}

	// Convert an RW pointer (as returned by alloc/xbyak) to its RX executable alias
	static void* ToExecutable(void* rwPtr)
	{
		return reinterpret_cast<uint8_t*>(rwPtr) - JITMemoryUtil::GetWritableOffset();
	}
#else
	// Standard allocator for macOS/Linux/Android
